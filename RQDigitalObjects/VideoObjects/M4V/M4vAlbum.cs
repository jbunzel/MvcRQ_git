...
using RQDigitalObjects.VideoObjects; 

namespace RQDigitalObjects.VideoObjects.M4V
{
	
    /// <summary>
	/// Diese Klasse liest ein Verzeichnis mit M4V-Dateien
	/// </summary>
	public class M4vAlbum : StructuredDigitalObject
	{
...
#region public constructors
        ...
        /// <summary>
        /// Constructs a M4V-Album.
        /// </summary>
        /// <param name="sourcePath">
        /// Path to the source folder of the M4V-Album.
        /// (The source folder is assumed to be a subfolder of the video media server (default: http://[hostname]/MyVideo)).
        /// </param>
        public M4vAlbum(string path)
...
#region public methods
        ...
        /// IN:     Toc from RQItem.      
        /// OUT:    Toc string to be stored in RQItem containing access structures to the parts of the m4v album.
        /// </param>
        /// <param name="signatureStr">
        /// IN:     Signature from RQItem.
        /// OUT:    Signature string to be stored in RQItem containing the primary (i. e. first part) access link to the m4v album
        /// </param>
        ...
        public override bool BindTo(string itemId, ref string tocString, ref string signatureStr, StringDictionary itemDescription)
        {
            ...
            if (tocString.Contains("XXXXX_"))
            {
                ...
                if (this.ElementCount > 0)
                {
                    ...
                    string newSigStr = "JB: MyVideo=" + this.GenerateAccessLink(itemId) + "; ";
        ...
        public override string DigitalObjectIdentifier(DigitalObject.DOIdentifier IdType)
        {
            switch (IdType)
            {
                case DOIdentifier.filename:
                    return this.GetTagValue(actlTrack, "M4VFileName");
        ...
        {
            string strRegex = @"MyVideo=(?<res>(\d|X){5}_[_/\w]+\.\w{3});";
            ...
            {
                strRegex = @"(\w+: )?MyVideo=(?<res>(\d|X){5}_[_/\w]+(\.\w{3})?);\x20?";
                ...
                foreach (Match myMatch in myRegex.Matches(toTarget))
                    if ((myMatch.Success) && (myMatch.Groups["res"].Value == firstAdress))
                        return toTarget;
                toTarget = toTarget.Insert(0, "JB: MyVideo=" + firstAdress + "; ");
            }
            return toTarget;
        }
        ...
        {
            M4vAlbum newdo = (M4vAlbum)this.MemberwiseClone();
