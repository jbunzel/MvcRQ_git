...
namespace Mvc5RQ.Areas.UserManagement.Controllers
{
    ...
    public class RolesAdminController : Mvc5RQ.Controllers.BaseController
    {
        ...
        /// <summary>
        /// Returns a list of all currently defined roles
        /// </summary>
        /// <returns>All known membership roles</returns>
        public JsonResult GetAllRoles()
        {
            Array roles = _userAccountService.GetAllRoles();

            if (roles.Length == 0)
                throw new Exception(RQResources.Views.Shared.SharedStrings.um_no_roles_defined);
            else
            {
                return Json(roles);
            }
        ...
        public async Task<ActionResult> Delete(RoleViewModel roleViewModel)
        {
            if (ModelState.IsValid)
            {
                IdentityResult roleresult = _userAccountService.DeleteRole(roleViewModel.Name);
