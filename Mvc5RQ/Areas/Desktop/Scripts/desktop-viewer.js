function getDesktopList() {
    callAjax_ResultList(HostAdress() + "/rqdt");
    $(".result_segments").change(function (e) {
        var new_project = parseInt($("select option:selected").attr("value"));

        e.preventDefault();
        getProjectList(new_project);
    });
}

function exportDesktop() {
    var json = JSON.stringify(parseInt($("select option:selected").attr("value")));
    var url = HostAdress() + "/rqdt/export";
    ...
    $.ajax({
        ...
        success: function (data, textStatus, jqXHR) {
            ...
            var response = { isSuccess: true,
                message: "Der Desktop wurde in das zugehörige Strands-Verzeichnis exportiert."
            }
...
function getProjectList(projectNr) {
    var fd = new ajaxLoadingIndicator("#html");
    var targetUrl = HostAdress() + "/rqdt/" + projectNr;
...
function renderHtmlList(data) {
    ...
    //});
    for (i = 1; i <= Object.keys(data.ProjectTitles).length ; i++)
        optionsValues += '<option value="' + i +'">' + data.ProjectTitles[i] + '</option>';
