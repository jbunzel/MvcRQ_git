...
-->
  ...
  <xsl:template match="/">
    <xsl:choose>
      <xsl:when test="//rq:ArrayOfRQItem">
        <xsl:choose>
          <xsl:when test="$interface='SRU'">
            <xsl:element name="records">
              <xsl:apply-templates />
              <xsl:for-each select="rq:ArrayOfRQItem/rq:RQItem">
  ...
  <xsl:template name="DCRecord">
    <xsl:if test="rq:Title">
      <dc:title>
        <xsl:value-of select="rq:Title" />
      </dc:title>
    </xsl:if>
    <xsl:if test="rq:Authors">
      <xsl:variable name="Subfields">
        <xsl:call-template name="ParseSubFields">
          <xsl:with-param name="FieldSubString" select="concat(rq:Authors,';')" />
        </xsl:call-template>
      </xsl:variable>
    ...
    <xsl:if test="rq:Subjects | rq:IndexTerms | rq:AboutPersons">
      <!-- Classification not covered -->
      <xsl:variable name="Subfields">
        <xsl:call-template name="ParseSubFields">
          <xsl:with-param name="FieldSubString" select="concat(concat(rq:Subjects,';'),concat(rq:IndexTerms,';'),concat(rq:AboutPersons,';'))" />
        </xsl:call-template>
      </xsl:variable>
    ...
    <xsl:if test="rq:Abstract">
      <xsl:if test="not(contains(rq:Abstract,'$$'))">
        <dc:description>
          <xsl:value-of select="rq:Abstract" />
        </dc:description>
      </xsl:if>
    </xsl:if>
    <xsl:if test="rq:Publisher">
      <xsl:variable name="Subfields">
        <xsl:call-template name="ParseSubFields">
          <xsl:with-param name="FieldSubString" select="concat(rq:Publisher,';')" />
        </xsl:call-template>
      </xsl:variable>
    ...
    <xsl:if test="rq:Authors | rq:Institutions">
      <xsl:variable name="Subfields">
        <xsl:call-template name="ParseSubFields">
          <xsl:with-param name="FieldSubString" select="concat(concat(rq:Institutions,';'),concat(rq:Authors,';'))" />
        </xsl:call-template>
      </xsl:variable>
    ...
    <xsl:if test="rq:PublTime">
      <!-- CreateTime not covered -->
      <xsl:variable name="Subfields">
        <xsl:call-template name="ParseSubFields">
          <xsl:with-param name="FieldSubString" select="concat(rq:PublTime,';')" />
        </xsl:call-template>
      </xsl:variable>
    ...
    <xsl:if test="rq:DocTypeName | rq:WorkType">
      <xsl:variable name="Subfields">
        <xsl:call-template name="ParseSubFields">
          <xsl:with-param name="FieldSubString" select="concat(concat(rq:DocTypeName,';'),concat(rq:WorkType,';'))" />
        </xsl:call-template>
      </xsl:variable>
    ...
    <xsl:if test="rq:Pages">
      <dc:format>
        <xsl:value-of select="rq:Pages" />
      </dc:format>
    </xsl:if>
    <xsl:if test="rq:ISDN | rq:Coden">
      <xsl:variable name="Subfields">
        <xsl:call-template name="ParseSubFields">
          <xsl:with-param name="FieldSubString" select="concat(concat(rq:ISDN,';'),concat(rq:Coden,';'))" />
        </xsl:call-template>
      </xsl:variable>
    ...
    <xsl:if test="rq:Source">
      <xsl:variable name="Subfields">
        <xsl:call-template name="ParseSubFields">
          <xsl:with-param name="FieldSubString" select="concat(rq:Source,';')" />
        </xsl:call-template>
      </xsl:variable>
    ...
    <xsl:if test="rq:Language">
      <xsl:variable name="Subfields">
        <xsl:call-template name="ParseSubFields">
          <xsl:with-param name="FieldSubString" select="concat(rq:Language,';')" />
        </xsl:call-template>
      </xsl:variable>
    ...
    <xsl:if test="rq:Series">
      <xsl:variable name="Subfields">
        <xsl:call-template name="ParseSubFields">
          <xsl:with-param name="FieldSubString" select="concat(rq:Series,';')" />
        </xsl:call-template>
      </xsl:variable>
    ...
    <xsl:if test="rq:AboutLocation | rq:AboutTime">
      <xsl:variable name="Subfields">
        <xsl:call-template name="ParseSubFields">
          <xsl:with-param name="FieldSubString" select="concat(concat(rq:AboutLocation,';'),concat(rq:AboutTime,';'))" />
        </xsl:call-template>
      </xsl:variable>
    ...
    <xsl:if test="rq:RQRights">
      <dc:rights>
        <xsl:value-of select="rq:RQRights" />
      </dc:rights>
    </xsl:if>
  </xsl:template>
