<xsl:stylesheet xmlns="http://www.loc.gov/mods/v3" xmlns:marc="http://www.loc.gov/MARC21/slim"
	...
	exclude-result-prefixes="xlink marc" version="1.0" xmlns:srw="http://www.loc.gov/zing/srw/"
	xmlns:at="http://www.w3.org/2005/Atom" xmlns:dc="http://purl.org/dc/elements/1.1/"
	xmlns:rq="http://www.riquest.de/rq/">
	<!--
	<xsl:include href="SRUMARC21slimUtils.xsl"/>
-->
	<!--
...
-->
	...
	<!-- 1/12/08: jb deleted "marc:" prefix in scanned elements -->
...
-->
	<xsl:template match="/">
		<xsl:choose>
			<xsl:when test="/at:feed/at:entry">
				<modsCollection xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
					...
					<xsl:for-each select="//at:feed/at:entry">
						<mods version="3.2">
							<xsl:call-template name="OCLCAtom"/>
						</mods>
					</xsl:for-each>
				</modsCollection>
			</xsl:when>
			<xsl:when test="//srw:records">
				<modsCollection xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
					xsi:schemaLocation="http://www.loc.gov/mods/v3 http://www.loc.gov/standards/mods/v3/mods-3-2.xsd">
					<xsl:for-each select="//srw:record/srw:recordData/record">
						<mods version="3.2">
	...
	<xsl:template name="OCLCAtom">
		<xsl:for-each select="at:title">
			<titleInfo>
				<title>
					<xsl:value-of select="."/>
				</title>
		...
		<xsl:for-each select="at:author">
			<name type="personal">
				<namePart>
					<xsl:call-template name="chopPunctuation">
						<xsl:with-param name="chopString" select="at:name"/>
...
-->
		...
		<xsl:for-each select="dc:identifier">
			<xsl:element name="identifier">
				<xsl:variable name="ISDN">
					<xsl:choose>
						<xsl:when
							test="starts-with(.,'URN:') or starts-with(.,'urn:') or starts-with(.,'Urn:')">
							<xsl:value-of
								select="substring(.,string-length('URN:')+1,string-length(.)-string-length('URN:'))"
							/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="."/>
				...
				<xsl:if
					test="starts-with($ISDN,'ISBN:') or starts-with($ISDN,'isbn:') or starts-with($ISDN,'Isbn:')">
					<xsl:attribute name="type">
						<xsl:text>isbn</xsl:text>
					</xsl:attribute>
					<xsl:value-of
						select="substring($ISDN,string-length('ISBN:')+1,string-length($ISDN)-string-length('ISBN:'))"
					/>
				</xsl:if>
				<xsl:if
					test="starts-with($ISDN,'ISSN:') or starts-with($ISDN,'issn:') or starts-with($ISDN,'Issn:')">
					<xsl:attribute name="type">
						<xsl:text>issn</xsl:text>
					</xsl:attribute>
					<xsl:value-of
						select="substring($ISDN,string-length('ISSN:')+1,string-length($ISDN)-string-length('ISSN:'))"
					/>
				</xsl:if>
			</xsl:element>
		</xsl:for-each>
		<xsl:for-each select="at:link">
			<location type="uri">
			...
			<titleInfo type="uniform">
				<title>
					<xsl:variable name="str">
						<xsl:for-each select="subfield">
							<xsl:if
								test="(contains('adfklmor',@code) and (not(../subfield[@code='n' or @code='p']) or (following-sibling::subfield[@code='n' or @code='p'])))">
								<xsl:value-of select="text()"/>
								<xsl:text> </xsl:text>
							</xsl:if>
						</xsl:for-each>
			...
			<xsl:for-each select="datafield[@tag=260]/subfield[@code='a']">
				<place>
					<placeTerm>
						...
						<!-- 
Herausgenommen 100109, da die Anfangsklammer zur Kennzeichnung von Ortsangaben außerhalb des Titelblatts entfernt wird.    
						<xsl:call-template name="chopPunctuationFront">
							<xsl:with-param name="chopString">
-->
						<xsl:call-template name="chopPunctuation">
...
-->
	...
	<xsl:template name="isInvalid">
		<xsl:param name="type"/>
		<xsl:if test="subfield[@code='z'] or subfield[@code='y']">
			<identifier>
				<xsl:attribute name="type">
