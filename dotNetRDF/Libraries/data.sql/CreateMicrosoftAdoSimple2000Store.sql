  ...
  IF @id = 0
    BEGIN
      ...
      IF LEN(@graphUri) > 450
		SET @partialValue = SUBSTRING(@graphUri, 0, 449);
	  ELSE
	    SET @partialValue = @graphUri;
      INSERT INTO GRAPHS (graphUri, graphUriIndex) VALUES (@graphUri, @partialValue);
	...
	IF @id = 0
	  BEGIN
	    ...
	    IF LEN(@nodeValue) > 450
		  SET @partialValue = SUBSTRING(@nodeValue, 0, 449);
		ELSE
		  SET @partialValue = @nodeValue;
	    INSERT INTO NODES (nodeType, nodeValue, nodeValueIndex, nodeMeta) VALUES (@nodeType, @nodeValue, @partialValue, @nodeMeta);
	    RETURN SCOPE_IDENTITY();
	  END
	ELSE
	  RETURN @id;
...
BEGIN
	...
	ELSE
	  SET @partialValue = @nodeValue;
	INSERT INTO NODES (nodeType, nodeValue, nodeValueIndex) VALUES (0, @nodeValue, @partialValue);
	RETURN SCOPE_IDENTITY();
END
