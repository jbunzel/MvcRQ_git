...
namespace VDS.RDF.Web
{
    ...
    public class QueryHandler : BaseSparqlQueryHandler
    {
        ...
        protected override BaseQueryHandlerConfiguration LoadConfig(HttpContext context)
        {
            ...
            if (temp != null)
            {
                if (temp is BaseQueryHandlerConfiguration)
                {
                    return (BaseQueryHandlerConfiguration)temp;
                }
                else
                {
            ...
            if (configFile == null) throw new DotNetRdfConfigurationException("Unable to load Query Handler Configuration as the Web.Config file does not specify a 'dotNetRDFConfig' AppSetting to specify the RDF configuration file to use");
